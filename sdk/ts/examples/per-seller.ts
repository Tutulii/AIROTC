...
async function main() {
  ...
  const offer = await client.offers.create({
    asset: process.env.AIROTC_TRADE_ASSET || "SOL",
    mode: "sell",
    amount: Number(process.env.AIROTC_TRADE_AMOUNT || 1),
    price: Number(process.env.AIROTC_TRADE_PRICE_SOL || 0.1),
    collateral: Number(process.env.AIROTC_TRADE_COLLATERAL_SOL || 0.02),
    rollupMode: "PER",
  });

  console.log(`Posted PER sell offer ${offer.id}`);

  client.on("deal_matched", async (deal) => {
    ...
    await deal.sendEncryptedDelivery(
    ...
    console.log(`PER seller flow settled for ticket ${deal.id}`);
  });
}
